...
describe('Tests SDMX-ML structure parsers', function () {
    ...
    context('For SDMX 3.0', function () {
        ...
        let path = "./tests/resources/Structures/SDMX 3.0/Geospatial/";
        it('Tests parsers for Geographic codelists', async () => {
            let xmlMessage = fs.readFileSync(path + 'geospatial_geographiccodelist.xml', 'utf8');
            let sdmxObjects = await new SdmxXmlParser().getIMObjects(xmlMessage, apiVersion);
            assert(sdmxObjects instanceof SdmxStructureObjects);
            assert(sdmxObjects.sdmxObjects.get("CODE_LIST")[0] instanceof ItemSchemeObject);
        });
        it.only('Tests parsers for Geogrid codelists', async () => {
            let xmlMessage = fs.readFileSync(path + 'geospatial_geogridcodelist.xml', 'utf8');
            let sdmxObjects = await new SdmxXmlParser().getIMObjects(xmlMessage, apiVersion);
            assert(sdmxObjects instanceof SdmxStructureObjects);
            assert(sdmxObjects.sdmxObjects.get("CODE_LIST")[0] instanceof ItemSchemeObject);
        });
