    ...
    static getItems(structureType, sdmxJsonObject) {
        ...
        if (SDMX_STRUCTURE_TYPE.CODE_LIST.key === structureType) {
            ...
            if (geoType === "GeographicCodelist") {
                return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "GeoFeatureSetCode");
            } else if (geoType === "GeoGridCodelist") {
                return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "GeoGridCode");
            } else {
                return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "Code");
            }
        } else if (SDMX_STRUCTURE_TYPE.CONCEPT_SCHEME.key === structureType) {
            return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "Concept");
        } else if (SDMX_STRUCTURE_TYPE.CATEGORY_SCHEME.key === structureType) {
            return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "Category");
        } else if (SDMX_STRUCTURE_TYPE.AGENCY_SCHEME.key === structureType) {
            return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "Agency");
        } else if (SDMX_STRUCTURE_TYPE.DATA_PROVIDER_SCHEME.key === structureType) {
            return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "DataProvider");
        } else if (SDMX_STRUCTURE_TYPE.DATA_CONSUMER_SCHEME.key === structureType) {
            return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "DataConsumer");
        } else if (SDMX_STRUCTURE_TYPE.ORGANISATION_UNIT_SCHEME.key === structureType) {
            return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "OrganisationUnit");
        } else if (SDMX_STRUCTURE_TYPE.METADATA_PROVIDER_SCHEME.key === structureType) {
            return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "MetadataProvider"); //TODO hierarchies ???
        } else if (SDMX_STRUCTURE_TYPE.VALUE_LIST.key === structureType) {
            return SdmxXmlV30ItemsParser._getItems(sdmxJsonObject, "ValueItem");
        } else {
        ...
        if (isDefined(itemsJson)) {
            for (let i = 0; i < itemsJson.length; i++) {
                items.push(new ItemObject(itemsJson[i].$.id, 
                    SdmxXmlV30StructureReferencesParser.getReferences(itemsJson[i]),
                    SdmxXmlRepresentationParser.getRepresentation(itemsJson[i]),
                    this._getItemsParent(itemsJson[i])
                ));
                if (itemsJson[i][jsonKey]) {
                    SdmxXmlV30ItemsParser._getItems(itemsJson[i], jsonKey, items);
                }
            }
        }
        return items;
...
module.exports = SdmxXmlV30ItemsParser;