...
const STRUCTURES_REST_RESOURCE = {
        ...
        getStructureResources(apiVersion, requestMode) {
                // Minimum & Full modes do not filter resources: return all available resources for the given api version
                let allResources = this.getStructureRestResources(apiVersion);
                if (requestMode === TEST_REQUEST_MODE.MINIMUM
                        || requestMode === TEST_REQUEST_MODE.FULL) {
                        return allResources;
                }
                let profileResources = [];
                if (requestMode === TEST_REQUEST_MODE.BASIC) {
                        profileResources.push(this.datastructure);
                        profileResources.push(this.metadatastructure);
                        profileResources.push(this.categoryscheme);
                        profileResources.push(this.conceptscheme);
                        profileResources.push(this.codelist);
                        profileResources.push(this.agencyscheme);
                        profileResources.push(this.dataproviderscheme);
                        profileResources.push(this.dataflow);
                        profileResources.push(this.metadataflow);
                        profileResources.push(this.provisionagreement);
                        profileResources.push(this.categorisation);
                        profileResources.push(this.structure);
                        profileResources.push(this.hierarchicalcodelist);
                        profileResources.push(this.organisationscheme);
                        profileResources.push(this.contentconstraint);
                        profileResources.push(this.dataconstraint);
                } else if (requestMode === TEST_REQUEST_MODE.DATA_DISCOVERY) {
                        profileResources.push(this.dataflow);
                        profileResources.push(this.datastructure);
                        profileResources.push(this.conceptscheme);
                        profileResources.push(this.codelist);
                }
                return profileResources.filter(resource => allResources.includes(resource) );
        },
        getSchemaResources() {
                var resources = [];
        ...
        getStructureRestResources(apiVersion) {
                ...
                if (API_VERSIONS[apiVersion] <= API_VERSIONS["v1.5.0"]) {
