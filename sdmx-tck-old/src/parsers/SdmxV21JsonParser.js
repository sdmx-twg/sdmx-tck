import MaintainableObject from '../model/MaintainableObject';
import ItemSchemeObject from '../model/ItemSchemeObject';
import DataflowObject from '../model/DataflowObject';
import DataStructureObject from '../model/DataStructureObject';
import ContentConstraintObject from '../model/ContentConstraintObject';
import SDMX_STRUCTURE_TYPE from '../constants/SdmxStructureType';
import SdmxV21StructureReferencesParser from './SdmxV21StructureReferencesParser';
import SdmxV21JsonItemsParser from './SdmxV21JsonItemsParser';
...
class SdmxV21JsonParser {
    static parseStructures(sdmxJsonObjects) {
        ...
        if (sdmxJsonObjects && sdmxJsonObjects.Structure && sdmxJsonObjects.Structure.Structures[0]) {
            ...
            SdmxV21JsonParser.parseAgencySchemes(structures, s);
            SdmxV21JsonParser.parseOrganisationUnitSchemes(structures, s);
            SdmxV21JsonParser.parseDataProviderSchemes(structures, s);
            SdmxV21JsonParser.parseDataConsumerSchemes(structures, s);
            SdmxV21JsonParser.parseAttachmentConstraints(structures, s);
            SdmxV21JsonParser.parseContentConstraints(structures, s);
            SdmxV21JsonParser.parseCategorySchemes(structures, s);
            SdmxV21JsonParser.parseCodelists(structures, s);
            SdmxV21JsonParser.parseConceptSchemes(structures, s);
            SdmxV21JsonParser.parseDataflows(structures, s);
            SdmxV21JsonParser.parseHierarchicalCodelists(structures, s);
            SdmxV21JsonParser.parseDataStructures(structures, s);
            SdmxV21JsonParser.parseMetadataflows(structures, s);
            SdmxV21JsonParser.parseMetadataStructures(structures, s);
            SdmxV21JsonParser.parseProcesses(structures, s);
            SdmxV21JsonParser.parseStructureSets(structures, s);
            SdmxV21JsonParser.parseReportingTaxonomies(structures, s);
            SdmxV21JsonParser.parseCategorisations(structures, s);
            SdmxV21JsonParser.parseProvisionAgreements(structures, s);
    ...
    static parseDataflows(structures, s) {
        if (s.Dataflows && s.Dataflows[0] && s.Dataflows[0].Dataflow) {
            for (var d in s.Dataflows[0].Dataflow) {
                ...
                structures.get(structureType).push(
                    new DataflowObject(dataflow, SdmxV21StructureReferencesParser.getReferences(dataflow)));
            }
        }
    };
    ...
    static parseMetadataflows(structures, s) {
        if (s.Metadataflows && s.Metadataflows[0] && s.Metadataflows[0].Metadataflow) {
            ...
            for (var m in metadataflows) {
                ...
                structures.get(structureType).push(
                    new MaintainableObject(structureType, metadataflows[m], SdmxV21StructureReferencesParser.getReferences(metadataflows[m])));
            }
        }
    };
    static parseMetadataStructures(structures, s) {
        if (s.MetadataStructures && s.MetadataStructures[0] && s.MetadataStructures[0].MetadataStructure) {
            ...
            for (var m in msds) {
                ...
                structures.get(structureType).push(
                    new MaintainableObject(structureType, msds[m], SdmxV21StructureReferencesParser.getReferences(msds[m])));
            }
        }
    };
    static parseProcesses(structures, s) {
        if (s.Processes && s.Processes[0] && s.Processes[0].Process) {
            ...
            for (var p in processes) {
                ...
                structures.get(structureType).push(
                    new MaintainableObject(structureType, processes[p], SdmxV21StructureReferencesParser.getReferences(processes[p])));
            }
        }
    };
    static parseStructureSets(structures, s) {
        if (s.StructureSets && s.StructureSets[0] && s.StructureSets[0].StructureSet) {
            ...
            for (var c in structureSets) {
                ...
                structures.get(structureType).push(
                    new MaintainableObject(structureType, structureSets[c], SdmxV21StructureReferencesParser.getReferences(structureSets[c])));
            }
        }
    };
    static parseReportingTaxonomies(structures, s) {
        if (s.ReportingTaxonomies && s.ReportingTaxonomies[0] && s.ReportingTaxonomies[0].ReportingTaxonomy) {
            ...
            for (var r in rtx) {
                ...
                structures.get(structureType).push(
                    new MaintainableObject(structureType, rtx[r], SdmxV21StructureReferencesParser.getReferences(rtx[r])));
            }
        }
    };
    static parseCategorisations(structures, s) {
        if (s.Categorisations && s.Categorisations[0] && s.Categorisations[0].Categorisation) {
            ...
            for (var c in ctg) {
                ...
                structures.get(structureType).push(
                    new MaintainableObject(structureType, ctg[c], SdmxV21StructureReferencesParser.getReferences(ctg[c])));
            }
        }
    };
    static parseProvisionAgreements(structures, s) {
        if (s.ProvisionAgreements && s.ProvisionAgreements[0] && s.ProvisionAgreements[0].ProvisionAgreement) {
            ...
            for (var p in provisionAgreements) {
                ...
                structures.get(structureType).push(
                    new MaintainableObject(structureType, provisionAgreements[p], SdmxV21StructureReferencesParser.getReferences(provisionAgreements[p])));
            }
        }
    };
};

export default SdmxV21JsonParser;