...
import { setHttpResponseValidationResult, updateTestsStatus, passIdentifiersToChildren, increaseTestCompliantNumber, increaseTestCoverageNumber } from "../handlers/helperFunctions";
...
const testsManagerReducer = (state = [], action) => {
	switch (action.type) {
		case ACTION_NAMES.INITIALISE_MODEL:
			// let tests = prepareTests(action.data.endpoint, action.data.apiVersion, action.data.indices);
