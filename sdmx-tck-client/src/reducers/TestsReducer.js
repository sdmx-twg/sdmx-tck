...
import {configTests,getDataFromParent,updateTestsStatus,increaseRunTestsNum, passIdentifiersToChildren, increaseTestCompliantNumber, increaseTestCoverageNumber } from "../handlers/helperFunctions";
...
const testsManagerReducer = (state = [], action) => {
	switch (action.type) {
		...
		case ACTION_NAMES.CONFIG_SCHEMA_TESTS:
			return configTests(state,action)
