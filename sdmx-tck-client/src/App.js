...
function App() {
  ...
  const handleIndexChange = (indices) => {
    setIndices(indices);
    let mode = TEST_REQUEST_MODE.BASIC;
    if (!indices.includes(TEST_INDEX.Structure)) {
        mode = "";
    }
    handleRequestModeChange(mode); //reset request mode
  }
  ...
  return (
    <div id="appContent">
      ...
      <div id="buttons">
        ...
        />
        <ExportReport endpoint={endpoint} apiVersion={apiVersion} format={format} requestMode={requestMode} />
      </div>
