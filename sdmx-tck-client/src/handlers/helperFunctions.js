...
export const increaseRunTestsNum = (prevStore, action) => {
	var testsArray = [...prevStore.tests];
	
	increaseNumOfRunTests(testsArray, action.testIndex);
	
	return {...prevStore, tests: testsArray};
};

export const increaseNumOfRunTests = (testsArray, testIndex) => {
	for (let i = 0; i < testsArray.length; i++) {
		if (testsArray[i].id === testIndex) {
...
export function passDataToRegistrationTests(tests, data) {
	if (tests.subTests && Array.isArray(tests.subTests)) {
		for (let i = 0; i < tests.subTests.length; i++) {
			let subTest = tests.subTests[i];
			subTest.identifiers.registrationId = data.registrationId;
            subTest.identifiers.updatedBefore = data.lastUpdated;
			subTest.identifiers.updatedAfter = data.lastUpdated;
			
			let providerRef = data.providerRef;
			subTest.identifiers.providerAgency = providerRef.agencyId;
            subTest.identifiers.providerId = providerRef.identifiableIds[0];
			
			let context = subTest.reqTemplate.context;
			if (context && data[context]) {
				let structureRef = data[context].structureRef;
				if (structureRef) {
					subTest.identifiers.context = context;
					subTest.identifiers.agency = structureRef.agencyId;
					subTest.identifiers.id = structureRef.id;
					subTest.identifiers.version = structureRef.version;
				}
...
export const configRegistrationTests = (prevStore, action) => {
	var testsArray = [...prevStore.tests];

	for (let i = 0; i < testsArray.length; i++) {
		if (testsArray[i].id === TEST_INDEX.Registration) {
			passDataToRegistrationTests(testsArray[i], action.data);
		}
	}
	return {...prevStore, tests: testsArray};
}
...
function updateChildTestsStatus(testsArray, parentTest) {
	if (parentTest && parentTest.subTests) {
		for (let i = 0; i < parentTest.subTests.length; i++) {
			...
			increaseNumOfRunTests(testsArray, parentTest.subTests[i].testIndex);
