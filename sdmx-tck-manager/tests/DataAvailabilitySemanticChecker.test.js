...
const SdmxJsonParser = require('sdmx-tck-parsers/src/parsers/SdmxJsonParser.js');
...
describe('Tests data availability tests', function () {
    context('With api version v1.4.0', function () {
        ...
        describe('Tests single dimension', function () {
            it('Tests component=FREQ parameter', async () => {
                ...
                let parentObjects = await new SdmxXmlParser().getIMObjects(parentXmlMessage, format);
    ...
    context.only('With api version v2.0.0', function () {
        let apiVersion = "v2.0.0";
        let format = SDMX_MESSAGE_FORMAT.JSON_V200.key;
        describe('Tests parent test - ' + format, function () {
            it('It should assert semantic validation result', async () => {
                let xmlMessage = fs.readFileSync('./tests/resources/Data Availability/ECB_EXR_1.0/SDMX JSON 2.0/DataAvailabilityAll.json', 'utf8')
                let sdmxObjects = await new SdmxJsonParser().getIMObjects(xmlMessage, format);
                let test = {
                    apiVersion: apiVersion,
                    resource: "dataflow",
                    reqTemplate: {},
        ...
        describe('Tests mode parameter (exact, available)', async function () {
            let dsdObj;
            before(async () => {
                let dsdXml = fs.readFileSync('./tests/resources/Data Availability/ECB_EXR_1.0/SDMX JSON 2.0/ECB_ECB_EXR1_1.0.json', 'utf8');
                let dsdWorkspace = await new SdmxJsonParser().getIMObjects(dsdXml, format);
                dsdObj = dsdWorkspace.sdmxObjects.get('DSD')[0];
            });

            it('Tests single key (mode=exact) - ' + format, async () => {
                let xmlMessage = fs.readFileSync('./tests/resources/Data Availability/ECB_EXR_1.0/SDMX JSON 2.0/DataAvailabilitySingleKeyExact.json', 'utf8');
                let sdmxObjects = await new SdmxJsonParser().getIMObjects(xmlMessage, "JSON_V200");
                let test = {
                    apiVersion: apiVersion,
                    reqTemplate: { mode: "exact", keyInPath: true },
                    dsdObj: dsdObj
