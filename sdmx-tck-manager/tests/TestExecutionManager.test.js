...
describe("Runs specific tests", function () {
  context('With api version v1.4.0', function () {
    ...
    const format = SDMX_MESSAGE_FORMAT.XML_V21.key;
    
    it("Runs the /hierarchicalcodelist/agency/id/version/items test", async () => {
      ...
      await _executeTest(specificTest, format, endpoint);
    });
  ...
  context('With api version v2.0.0', function () {
    ...
    const format = SDMX_MESSAGE_FORMAT.XML_V300.key;
  ...
  const _executeTest = async (specificTest, format, endpoint) => {
    await TestExecutionManagerFactory.getTestsManager(specificTest.index)
      .executeTest(specificTest, format, endpoint)
      .then(
        (result) => {
