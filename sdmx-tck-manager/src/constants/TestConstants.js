...
function STRUCTURES_RESOURCE_IDENTIFICATION_PARAMETERES_SUPPORT(apiVersion, restResource) {
    let testsArray = [];
    STRUCTURE_IDENTIFICATION_PARAMETERS.getValues(apiVersion, restResource).forEach(parameter => {
...
function STRUCTURES_PARAMETERS_FOR_FURTHER_DESCRIBING_THE_RESULTS(apiVersion,resource,requestMode) {
    ...
    for (let i = 0; i < detailValues.length; i++) {
        if (detailValues[i] === STRUCTURE_QUERY_DETAIL.REFERENCE_PARTIAL) {
...
function DATA_REPRESENTATION_SUPPORT_TESTS(apiVersion){
    ...
    DATA_RESPRESENTATION_SUPPORT.getDataRepresentationSupportParameters(apiVersion).forEach(parameter => {
        testsArray.push({ index: "Data", key: parameter.key, url: parameter.url, reqTemplate: parameter.template })
    });
