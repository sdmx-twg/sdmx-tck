...
class RegistrationSemanticChecker {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (test.testType === TEST_TYPE.REGISTRATION_IDENTIFICATION_PARAMETERS) {
                    validation = RegistrationSemanticChecker._checkIdentification(test, preparedRequest, workspace, format)
                }
                resolve(validation);
            } catch (err) {
    ...
    static _checkIdentification(test, preparedRequest, workspace, format) {
        ...
        if (test.reqTemplate.byId === true) {
            ...
            return RegistrationSemanticChecker.checkProvisionAgreements(test, query, service, workspace, format);
        }
    ...
    static checkProvisionAgreements(test, query, service, workspace, format) {
        ...
        workspace.getSdmxObjectsList().forEach(async (registration) => {
            ...
            if (test.reqTemplate.byProvider === true) {
                isPraRefValid = await RegistrationSemanticChecker._isValidProviderRef(query, service, format, praRef);
                ...
                if (query.context === DATA_CONTEXT.provisionagreement) {
                    ...
                    isPraRefValid = await RegistrationSemanticChecker._isValidDataflowRef(query, service, format, praRef);
                } else if (query.context === DATA_CONTEXT.datastructure) {
                    isPraRefValid = await RegistrationSemanticChecker._isValidDatastructureRef(query, service, format, praRef);
                }
            }
            if (!isPraRefValid) {
    ...
    static async _isValidProviderRef(query, service, format, praRef) {
        // Get the dataflow referenced by the provision agreement
        let refs = await RegistrationTestsDataBuilder.getChildrenRefs(service.url, service.api, format, praRef);
    ...
    static async _isValidDataflowRef(query, service, format, praRef) {
        // Get the dataflow referenced by the provision agreement
        let refs = await RegistrationTestsDataBuilder.getChildrenRefs(service.url, service.api, format, praRef);
    ...
    static async _isValidDatastructureRef(query, service, format, praRef) {
        // Get the datastructure referenced by the dataflow
        let refs = await RegistrationTestsDataBuilder.getChildrenRefs(service.url, service.api, format, praRef, true);
