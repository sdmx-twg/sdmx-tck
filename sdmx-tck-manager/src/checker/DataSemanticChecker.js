...
class DataSemanticChecker {
    ...
    static _checkResourceIdentification(test, query, workspace) {
        ...
        if (identificationValidation.status === FAILURE_CODE) { 
    ...
    static _checkIdentification(test, query, workspace) {
        ...
        for (let dataset of workspace.getDatasets()) {
            let refs = dataset.getLinkedStructureRefs();
            let result = refs.some((ref) => {
                if (Utils.isSpecificAgency(reformedQuery)
                    && Utils.isSpecificId(reformedQuery)
                    && Utils.isSpecificVersion(reformedQuery)) {
                    
                    if (reformedQuery.version !== Utils.getLatestStableOperator(test.apiVersion)) {
                        if (ref.getAgencyId() === reformedQuery.agency
                            && ref.getId() === reformedQuery.id
                            && ref.getVersion() === reformedQuery.version) {
                            return true;
                        }
                    } else {
                        if (ref.getAgencyId() === reformedQuery.agency 
            ...
            if (result === false) {
                return {
                    status: FAILURE_CODE,
                    error: "Error in Identification: Some of the datasets do not reference the requested dataflow."
                }
            }
        }
        return { status: SUCCESS_CODE }
