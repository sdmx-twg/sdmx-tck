...
class DataTestsExecutionManager {
    static async executeTest(toRun, format, endpoint) {
        ...
        try {
            ...
            let preparedRequest = await DataRequestBuilderFactory.getBuilder(toRun.apiVersion).prepareRequest(endpoint, format, toRun)
            ...
            if (toRun.testType === TEST_TYPE.DATA_REPRESENTATION_SUPPORT_PARAMETERS || toRun.testType === TEST_TYPE.DATA_OTHER_FEATURES) {
                ...
                if(toRun.testType === TEST_TYPE.DATA_REPRESENTATION_SUPPORT_PARAMETERS){
                    httpResponseHeadersValidation = await ResponseValidator.validateRepresentation(toRun.reqTemplate.representation, httpResponse, toRun.apiVersion);
                }else{
            ...
            let workspace = await new SdmxParser().getIMObjects(response, format);
            if (!workspace) {
