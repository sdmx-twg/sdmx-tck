    ...
    static async executeTest(toRun, format, endpoint) {
        ...
        try {
            ...
            if (toRun.testType === TEST_TYPE.STRUCTURE_QUERY_REPRESENTATION) {
                let httpResponseHeadersValidation = await ResponseValidator.validateRepresentation(toRun.reqTemplate.representation, httpResponse, toRun.apiVersion);
            ...
            let workspace = await new SdmxParser().getIMObjects(await httpResponse.text(), format);
            if (!workspace) {
