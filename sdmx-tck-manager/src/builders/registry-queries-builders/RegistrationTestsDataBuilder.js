...
class RegistrationTestsDataBuilder {
    static async buildData(endpoint, apiVersion) {
        // 1. Reguest all registrations and pick randomly one.
        let registrationObject = await RegistrationTestsDataBuilder.getRandomRegistration(endpoint, apiVersion);
        
        // 2. Request and get the PRA referenced by the selected registration.
        let praRef = registrationObject.getProvisionAgreementRef();
        let refs = await RegistrationTestsDataBuilder.getChildrenRefs(endpoint, apiVersion, praRef, true);
        let data = {
    ...
    static async getRandomRegistration(endpoint, apiVersion) {
        let params = {
        ...
        let workspace = await HelperManager.getWorkspace(TestObjectBuilder.getTestObject(params), apiVersion, endpoint);
        
        return workspace.getRandomSdmxObjectOfType(SDMX_STRUCTURE_TYPE.REGISTRATION.key);
    }

    static async getChildrenRefs(endpoint, apiVersion, praRef, descendants) {
        let praObject = await RegistrationTestsDataBuilder.getStructure(endpoint, apiVersion, STRUCTURES_REST_RESOURCE.provisionagreement, praRef);
        ...
        if (descendants === true) {
            ...
            let dfObject = await RegistrationTestsDataBuilder.getStructure(endpoint, apiVersion, STRUCTURES_REST_RESOURCE.dataflow, dfRef);
    ...
    static async getStructure(endpoint, apiVersion, resource, structureRef) {
        // TEMPORAL SOLUTION:
        ...
        let params = {
            ...
            reqTemplate: {
                ...
                representation: structureFormat,
